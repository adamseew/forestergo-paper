...
\begin{figure}[t!]
  ...
  \begin{minipage}[t!]{.31\columnwidth}
    \caption[.]{\textbf{Outdoors model of a large-scale vegetated~area featuring our ergodic controller}. The~figure illustrates the UAV's trajectory over a 10~000 square-meter area located in Birmensdorf,~Switzerland. The~area features both dense vegetation and an agricultural setting. The dark red line represents the exploration history, while the dark red dot marks the~~UAV's~~cur-
    }
    \vspace*{-.2cm}
    \label{fig:2}
  \end{minipage}
