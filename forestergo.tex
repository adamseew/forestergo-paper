...
\title{Ergodic Control for Robotic Exploration with an\\Adaptive Gaussian Mixture Model}
...
{Seewald \MakeLowercase{\textit{et al.}}: Ergodic Control with an Adaptative Gaussian Mixture Model}

\maketitle

%\vspace*{.5cm}
\begin{abstract}
  The problem of exploring unknown spaces with robots arises in many real-world applications. While different approaches exist, those utilizing sensory data, e.g., vision, positioning, etc., have emerged as of particular interest. Among these, ergodic control is significant, as it accounts for both motion cost and optimality. However, ergodic controllers alone often encounter limitations when applied to unknown areas. This paper introduces a general and model agnostic ergodic control methodology designed to overcome these limitations. Unlike traditional ergodic controllers that rely on predefined information measures, our approach dynamically derives an information measure from sensory data and derives a degree of obstacle avoidance as a ``by-product.'' Empirical data from simulations in diverse environments, including those featuring dense vegetation, precision agriculture, etc., demonstrate the effectiveness of our model agnostic ergodic controller, showcasing the applicability of our methodology compared to conventional techniques typically restricted to structured spaces or spaces where an information measure is readily available.
\end{abstract}
...
Ergodic control is a planning and controls methodology that derives robot trajectories maximizing a given information measure so that robots spend more time in areas with high information measure while quickly traversing areas with low information measure~\cite{mathew2011metrics,abraham2017ergodic,miller2013trajectory}. As a consequence, it is required that the user provides an information measure, a priori or that an \textit{existing} information measure is refined as the robots gather more information about their surroundings. While applicable to some use cases, this is often a limitation of the existing ergodic controllers. It is not always the case that the information measure can be easily refined from the gathered data, or that an a priori information measure can be provided at all. Furthermore, it is also not always feasible to define an information measure for large-scale and/or unknown spaces in practice, where with ``large-scale'' we refer to spaces on the order of dozens or even hundreds of meters. 

\begin{figure}
  \centering
  \input{figures/ilabs.pdf_tex}
  \caption{\textbf{Illustrative use case of our model agnostic ergodic controller}. A drone explores an unknown state space using an Adaptative Gaussian Mixture Model, with the information density (3D plot) derived from the exploration history (points $\mathbf{x}(t-h)$ to $\mathbf{x}(t)$), dynamically avoiding obstacles (the tree).}
  \label{fig:-1}
  \vspace*{-.1cm}
\end{figure}
...
In the following sections, Sec.~\ref{sec:res1} presents experimental results from a simulated forest with an area of 3600~square~meters. Sec.~\ref{sec:res2} details our findings regarding the built-in obstacle avoidance capabilities of our general scaled ergodic controller compared to its coverage performance and discusses how the coverage evolves in time. Sec.~\ref{sec:res3} describes the outdoor modeled results, featuring an area with dense vegetation in an agricultural field, including multiple obstacles and a total area of 10~000 square meters. Sec~\ref{sec:res4} presents our comparative experimental results utilizing another ergodic controller in the literature~\cite{calinon2020mixture}.
...
The data demonstrate that our method inherently incorporates a tunable degree of obstacle avoidance. However, in practice, even low collision rates could lead to mission-critical failures. It is thus noted that our method still requires an external obstacle avoidance methodology in use cases where collisions lead to critical failures. Nonetheless, compared to other methods in the literature, our approach maintains optimality (ergodicity) across the majority of the state space, thereby \textit{significantly reducing} the reliance on external obstacle avoidance techniques.
...
\begin{figure}[t!]
  ...
  \begin{minipage}[t!]{.31\columnwidth}
    \caption[.]{\textbf{Outdoor model of a large-scale vegetated~area featuring our ergodic controller}. The~figure illustrates the UAV's trajectory over a 10~000 square-meter area located in Birmensdorf,~Switzerland. The~area features dense vegetation in an agricultural field. The dark red line represents the exploration history, while the dark red dot marks the~~UAV's~~current position. The prob- 
    }
    \vspace*{-.2cm}
    \label{fig:2}
  \end{minipage}
...
\noindent
Additional experimental evaluation is conducted using a model of an outdoor vegetated area located in Birmensdorf, ZH, Switzerland, with coordinates 47.362646\textdegree~latitude and 8.457228\textdegree~longitude. The area was selected to include dense vegetation in an agricultural field, simulating a real-world use case for our general adaptive ergodic controller. The exploration area is bounded to 10~000 square meters, and the surrounding region has been manually surveyed to assess future feasibility.
