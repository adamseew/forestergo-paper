...
\begin{figure}
  ...
  \caption{\textbf{Illustrative use case of a general scaled ergodic controller}. A drone explores an unknown state space using an adaptative Gaussian mixture model, with the information density (3D plot) derived from the exploration history (points $\mathbf{x}(t-h)$ to $\mathbf{x}(t)$), dynamically avoiding obstacles (the tree).}
  \label{fig:-1}
  \vspace*{-.1cm}
\end{figure}
...
\begin{figure}[b!]
  ...
  \begin{minipage}[t!]{.5\columnwidth}
    \vspace*{-.3cm}
    \input{figures/cov_vs_time.pdf_tex}
  \end{minipage}
\end{figure}
...
Fig.~\ref{fig:time-vs-cov} shows the evolution over coverage in time for all the history windows in Fig.~\ref{fig:0}. Larger history windows tend to have a lower initial coverage as more trajectort hisory data have are processed to derive the adaptive ergodic controller, but they eventually lead to a more stable coverage rate as the exploration continues.
